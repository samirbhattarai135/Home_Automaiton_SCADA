...
const UNKNOWN_DATA_TYPE_IN_ARGUMENT_MAP = "Unknown data type in argument map definition.";
const OFFSET_POSITION_INVALID = "The supplied offset for one of the fields in the argument map want's to read at the wrong position in memory";
const MISSING_MAPS = "Tried to call a custom function code without supplying at least one argument map, every custom function code needs at least a request map or a response map.";
const MISSING_ARGUMENT_MAP_PROPERTY = "One of the defined data types in your argument map is missing a required field check if all data types have their required fields.";
const REQUEST_ARGUMENT_MISSING_DATA = "One of the arguments in the Custom Functioncode request is missing it's data field.";
const NAME_ALREADY_DEFINED = "There is already some data with the same name please make sure that your data has a unique name in the response argument map.";
...
const UnknownDataType = function() {
    this.name = this.constructor.name;
    this.message = UNKNOWN_DATA_TYPE_IN_ARGUMENT_MAP;
    this.errno = null;
};

const MissingArgumentMapsError = function() {
    this.name = this.constructor.name;
    this.message = MISSING_MAPS;
    this.errno = null;
};

const OffsetPositionInvalid = function() {
    this.name = this.constructor.name;
    this.message = OFFSET_POSITION_INVALID;
    this.errno = null;
};

const MissingArgumentMapProperty = function() {
    this.name = this.constructor.name;
    this.message = MISSING_ARGUMENT_MAP_PROPERTY;
    this.errno = null;
};

const ArgumentWithoutData = function() {
    this.name = this.constructor.name;
    this.message = REQUEST_ARGUMENT_MISSING_DATA;
    this.errno = null;
};

const NameAlreadyDefined = function() {
    this.name = this.constructor.name;
    this.message = NAME_ALREADY_DEFINED;
    this.erno = null;
};
/**
...
/**
 * Compares the Count field of FC(1,2,3,4) Responses and compares it with the
 * remaining length of the data buffer.
 * @param {number} functionCode the received functionCode
 * @param {Buffer} data the data buffer to get the count field from.
 * @returns {boolean} returns true if the lengths are matching
 * @private
 */
function _checkInnerLength(functionCode, data) {

    if(functionCode > 1 && functionCode <= 4) {
        const byteCount = data.readUInt8(2);
        const dataBytes = data.slice(3, -2);

        return byteCount === dataBytes.length;
    }

    // NOTE: We don't know if the received message carries an internal byte count
    //      so we cannot safely determine the size we just assume the message
    //      length is correct!
...
function _readFC3or4(data, next) {
    const length = data.readUInt8(2);
    const registerData = data.slice(3, -2);

    const contents = [];

    for (let i = 0; i < registerData.length; i += 2) {
        const reg = registerData.readUInt16BE(i);
        contents.push(reg);
    }
...
function _readCustomMessage(modbus, data, argumentMap, next) {
    // const unitId = data.readUint8(0);
    // const functionCode = data.readUint8(1);

    const readArguments = ModbusRTU.readFCArguments(data, argumentMap, next, 2);
    next(null, readArguments);
...
function _onReceive(data) {
    ...
    if (!_checkInnerLength(code, data)) {
        error = "The byte count of code " + code + " with transaction id " + modbus._port._transactionIdRead +
            " is not correct! cannot parse the message without a crash ignoring it!";
        if(transaction.next)
            next(new Error(error));
    }
    ...
    */
    ...
    switch (code) {
        ...
        case 43:
            ...
            break;
        default:
            if(transaction.argumentMap !== undefined && transaction.argumentMap !== null && Object.keys(transaction.argumentMap).length !== 0) {
                _readCustomMessage(modbus, data, transaction.argumentMap, next);
            } else {
                next(new MissingArgumentMapsError());
            }
    }
}
...
class ModbusRTU extends EventEmitter {
    ...
    _sendCustomFc(address, customCode, customRequestArgumentMap, customResponseArgumentMap, next) {
        if(this.isOpen !== true) {
            if (next) next(new PortNotOpenError());
            return;
        }

        if(customRequestArgumentMap === null && customResponseArgumentMap === null) {
            if (next) next(new MissingArgumentMapsError());
            return;
        }

        if(!ModbusRTU._validateArgumentMap(customRequestArgumentMap, true, next)  || !ModbusRTU._validateArgumentMap(customResponseArgumentMap, false, next)) {
            return;
        }
        ...
        this._transactions[this._port._transactionIdWrite] = {
            ...
            nextCode: customCode,
            lengthUnknown: true,
            argumentMap: customResponseArgumentMap,
            next: next
        };

        const CRC_BYTES = 2;

        const codeLength = 2 + ModbusRTU._calculateArgumentMapLength(customRequestArgumentMap) + CRC_BYTES;

        const buf = Buffer.alloc(codeLength);

        buf.writeUint8(address);
        buf.writeUint8(customCode, 1);
        let currentPosition = 2;
        currentPosition += ModbusRTU.writeFCArguments(customRequestArgumentMap, currentPosition, buf, next);
        buf.writeUInt16LE(crc16(buf.slice(0, -2)), currentPosition);

        _writeBufferToPort.call(this, buf, this._port._transactionIdWrite);
    }
    // NOTE(Kay): Every argument map should at least have a name, offset and type
    static _validateArgumentMap(argumentMap, withPayload, next) {
        let offset = 0;
        ...
        if(argumentMap === null || argumentMap === undefined) return true;

        for(let i = 0; i < argumentMap.length; i++) {
            const descriptor = argumentMap[i];
            const hasName = Object.prototype.hasOwnProperty.call(descriptor, "name");
            const hasOffset = Object.prototype.hasOwnProperty.call(descriptor, "offset");
            const hasType = Object.prototype.hasOwnProperty.call(descriptor, "type");
            const hasData = Object.prototype.hasOwnProperty.call(descriptor, "data");
            if(hasName && hasOffset && hasType) {
                const userSuppliedOffset = descriptor.offset;

                if(withPayload === true && !hasData) {
                    if(next) next(new ArgumentWithoutData());
                    return;
                }

                if(userSuppliedOffset !== offset) {
                    if(next) next(new OffsetPositionInvalid());
                    return;
                }
                ...
                switch(descriptor.type) {
                    ...
                    case "uint32be":
                        ...
                        break;
                    default:
                        if(next) next(new UnknownDataType());
                        return false;
                }
            } else {
                // TODO(Kay): if the descriptor is missing any of these props then we have a error state and should inform the user about it !
                if(next) next(new MissingArgumentMapProperty());
                return false;
            }
        }
    ...
    static _calculateArgumentMapLength(argumentMap) {
        let size = 0;
        ...
        if(argumentMap === null || argumentMap === undefined) return 0;

        for(let i = 0; i < argumentMap.length; i++) {
            const descriptor = argumentMap[i];
            const datatype = descriptor.type;
            switch(datatype) {
                case "uint8be":
                    size += BYTE_SIZE;
                    break;
    ...
    static readFCArguments(data, argumentMap, next, startAt) {
        if(Object.keys(argumentMap).length === 0) {
            return; // NOTE(Kay): In the case of an unknown function code we need this map so we should throw an error here !
        }
        ...
        for(let i = 0; i < argumentMap.length; i++) {
            ...
            if(offset === nextReadPosition) {
                const bytesRead = this.readArgumentFromOffset(result, data, name, startAt + offset, type);
    ...
    static readArgumentFromOffset(result, data, name, offset, type, next) {
        const hasName = Object.prototype.hasOwnProperty.call(result, "name");
        if(hasName) {
            if (next) next(new NameAlreadyDefined());
            return; // TODO(Kay): If the name is already in our map we should go into a error state here !
        }
    ...
    static writeFCArguments(argumentMap, startAt, buf, next) {
        let bytesWritten = 0;

        if(argumentMap === null) return bytesWritten;

        for(let i = 0; i < argumentMap.length; i++) {
            const argument = argumentMap[i];
            ...
            switch(argument.type) {
                case "uint8be":
                    buf.writeUInt8(data, startAt + offset);
                    bytesWritten += 1;
                    break;
                case "uint16be":
                    buf.writeUInt16BE(data, startAt + offset);
                    bytesWritten += 2;
                    break;
                case "uint32be":
                    buf.writeUInt32BE(data, startAt + offset);
                    bytesWritten += 4;
                    break;
                default:
                    if(next) next(new UnknownDataType());
